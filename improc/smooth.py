...
def median_blur(img, ksize=5):
    ...
    # for example: 3, 5, 7
    # Here, the function cv2.medianBlur() takes median of all the pixels
    # under kernel area and central element is replaced with this median
    # value. This is highly effective against salt-and-pepper noise in the
    # images. Interesting thing is that, in the above filters, central
    # element is a newly calculated value which may be a pixel value in the
    # image or a new value. But in median blurring, central element is always
    #  replaced by some pixel value in the image. It reduces the noise
